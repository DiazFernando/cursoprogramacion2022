...
let cantidadMáximos: number = 0;  // variable para cantidad de valores máximos ingresados
...
while (valorIngresado !== 0) {
  if (valorIngresado > 0) { //si el valor ingresado es mayor a 0
    cantidadMáximos++;  // suma 1 por cada vuelta en la variable de cantidad maximos
  }
...
if (valoresTotales > 0) {  // si la variable valoresTotales es mayor a 0 realizar el calculo del porcentaje
  porcentajeDeMáximos = (cantidadMáximos * 100) / valoresTotales; // 
  console.log(
    cantidadMáximos + " valores positivos, " + porcentajeDeMáximos + "% del total de valores ingresados");
}
